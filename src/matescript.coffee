###
MateScript can be used both on the server, as a command-line compiler based
on Node.js/V8. This module contains the main entry functions for tokenizing,
parsing, and compiling source MateScript into PHP.
###
...
# The current MateScript version number.
exports.VERSION = '1.6.3'

fileExtensions = ['.mate', '.litmate', '.mate.md']
...
# Compile MateScript code to PHP, using the Coffee/Jison compiler.
#
...
exports.compile = compile = withPrettyErrors (code, options) ->
  ...
  for fragment in fragments
    ...
    # Copy the code from each fragment into the final PHP.
    js += fragment.code

  if options.header
    header = "Generated by MateScript #{@VERSION}"
    js = "<?php\n// #{header}\n\n#{js}"
...
# Tokenize a string of MateScript code, and return the array of tokens.
exports.tokens = withPrettyErrors (code, options) ->
  lexer.tokenize code, options

# Parse a string of MateScript code or an array of lexed tokens, and
...
# Compile and execute a string of MateScript (on the server), correctly
...
# Compile and evaluate a string of MateScript (in a Node.js-like environment).
# The MateScript REPL uses this to run the input.
...
compileFile = (filename, sourceMap) ->
  ...
  try
    ...
    # from the original file compiled with MateScript.run, add that
    # information to error so it can be pretty-printed later.
    throw helpers.updateSyntaxError err, stripped, filename

  answer

# Load and run a MateScript file for Node, stripping any `BOM`s.
loadFile = (module, filename) ->
  answer = compileFile filename, false
  module._compile answer, filename

# If the installed version of Node supports `require.extensions`, register
# MateScript as an extension.
...
# to fork both MateScript files, and PHP files, directly.
if child_process
  {fork} = child_process
  binary = require.resolve '../../bin/mate'
...
# Based on [michaelficarra/MateScriptRedux](http://goo.gl/ZTx1p)
# NodeJS / V8 have no support for transforming positions in stack traces using
# sourceMap, so we must monkey-patch Error to display MateScript source
# positions.
