...
(function() {
  ...
  SWITCHES = [['-b', '--bare', 'compile without a top-level function wrapper'], ['-c', '--compile', 'compile to PHP and save as .js files'], ['-e', '--eval', 'pass a string from the command line as input'], ['-h', '--help', 'display this help message'], ['-i', '--interactive', 'run an interactive MateScript REPL'], ['-j', '--join [FILE]', 'concatenate the source MateScript before compiling'], ['-m', '--map', 'generate source map and save as .map files'], ['-n', '--nodes', 'print out the parse tree that the parser produces'], ['--nodejs [ARGS]', 'pass options directly to the "node" binary'], ['-o', '--output [DIR]', 'set the output directory for compiled PHP'], ['-p', '--print', 'print out the compiled PHP'], ['-s', '--stdio', 'listen for and compile scripts over stdio'], ['-l', '--literate', 'treat stdio as literate style matescript'], ['-t', '--tokens', 'print out the tokens that the lexer/rewriter produce'], ['-v', '--version', 'display the version number'], ['-w', '--watch', 'watch scripts for changes and rerun commands']];
  ...
  compileScript = function(file, input, base) {
    ...
    try {
      ...
      if (o.tokens) {
        return printTokens(MateScript.tokens(t.input, t.options));
      } else if (o.nodes) {
        return printLine(MateScript.nodes(t.input, t.options).toString().trim());
      } else if (o.run) {
        return MateScript.run(t.input, t.options);
      ...
      if (MateScript.listeners('failure').length) {
        return;
      }
  ...
  version = function() {
    return printLine("MateScript version " + MateScript.VERSION);
  };
